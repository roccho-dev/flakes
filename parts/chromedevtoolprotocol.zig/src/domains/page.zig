...
pub const Page = struct {
    ...
    pub fn enable(self: *Self) !void {
        try self.session.sendCommandVoid("Page.enable", .{});
    }

    /// Disable page domain
    pub fn disable(self: *Self) !void {
        try self.session.sendCommandVoid("Page.disable", .{});
    }
    ...
    pub fn navigate(self: *Self, allocator: std.mem.Allocator, url: []const u8) !NavigateResult {
        ...
        defer self.session.deinitCommandResult(&result);
    ...
    pub fn reload(self: *Self, ignore_cache: ?bool) !void {
        try self.session.sendCommandVoid("Page.reload", .{
    ...
    pub fn goBack(self: *Self) !bool {
        ...
        try self.session.sendCommandVoid("Page.navigateToHistoryEntry", .{
            .entryId = blk: {
    ...
    pub fn stopLoading(self: *Self) !void {
        try self.session.sendCommandVoid("Page.stopLoading", .{});
    }
    ...
    pub fn setLifecycleEventsEnabled(self: *Self, enabled: bool) !void {
        try self.session.sendCommandVoid("Page.setLifecycleEventsEnabled", .{
            .enabled = enabled,
        });
    }
    ...
    pub fn addScriptToEvaluateOnNewDocument(self: *Self, source: []const u8) ![]const u8 {
        ...
        defer self.session.deinitCommandResult(&result);
        return try self.session.allocator.dupe(u8, try json_util.getString(result, "identifier"));
    }

    /// Remove script to evaluate on new document
    pub fn removeScriptToEvaluateOnNewDocument(self: *Self, identifier: []const u8) !void {
        try self.session.sendCommandVoid("Page.removeScriptToEvaluateOnNewDocument", .{
    ...
    pub fn bringToFront(self: *Self) !void {
        try self.session.sendCommandVoid("Page.bringToFront", .{});
    }

    /// Set document content
    pub fn setDocumentContent(self: *Self, html: []const u8) !void {
        try self.session.sendCommandVoid("Page.setDocumentContent", .{
            .html = html,
        });
    }
};
