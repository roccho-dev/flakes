...
pub const Target = struct {
    ...
    pub fn createTarget(self: *Self, url: []const u8) ![]const u8 {
        ...
        return try self.connection.allocator.dupe(u8, try json_util.getString(result, "targetId"));
    }
    ...
    pub fn activateTarget(self: *Self, target_id: []const u8) !void {
        try self.connection.sendCommandVoid("Target.activateTarget", .{
            .target_id = target_id,
        }, null);
    }
    ...
    pub fn setDiscoverTargets(self: *Self, discover: bool) !void {
        try self.connection.sendCommandVoid("Target.setDiscoverTargets", .{
            .discover = discover,
        }, null);
    }
    ...
    pub fn createBrowserContext(self: *Self) ![]const u8 {
        ...
        defer self.connection.deinitCommandResult(&result);
        return try self.connection.allocator.dupe(u8, try json_util.getString(result, "browserContextId"));
    }

    /// Dispose a browser context
    pub fn disposeBrowserContext(self: *Self, context_id: []const u8) !void {
        try self.connection.sendCommandVoid("Target.disposeBrowserContext", .{
