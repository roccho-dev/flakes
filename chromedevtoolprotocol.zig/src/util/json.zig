...
/// Deep-clone an owned JSON value.
pub fn cloneValue(allocator: std.mem.Allocator, value: std.json.Value) !std.json.Value {
    switch (value) {
        ...
        .array => |arr| {
            ...
            errdefer {
                for (new_arr.items) |*item| deinitValue(allocator, item);
                new_arr.deinit();
            }
        ...
        .object => |obj| {
            ...
            while (iter.next()) |entry| {
                ...
                var cloned = try cloneValue(allocator, entry.value_ptr.*);
                errdefer deinitValue(allocator, &cloned);
...
/// Release all heap storage owned by a JSON value allocated via this module.
...
/// Create an empty owned JSON object value.
...
pub fn stringifyValueToString(allocator: std.mem.Allocator, value: anytype) ![]const u8 {
    ...
    // Pass through dynamic JSON values as-is.
    if (T == std.json.Value) {
...
test "decode struct from json value" {
    ...
    const result = try decode(TestStruct, parsed.value, std.testing.allocator);
