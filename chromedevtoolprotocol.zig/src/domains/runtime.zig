...
pub const Runtime = struct {
    ...
    pub fn enable(self: *Self) !void {
        try self.session.sendCommandVoid("Runtime.enable", .{});
    }

    /// Disable runtime domain
    pub fn disable(self: *Self) !void {
        try self.session.sendCommandVoid("Runtime.disable", .{});
    }
    ...
    pub fn getProperties(self: *Self, allocator: std.mem.Allocator, object_id: []const u8, own_properties: ?bool) ![]PropertyDescriptor {
        ...
        var properties: std.ArrayList(PropertyDescriptor) = .empty;
        errdefer properties.deinit(allocator);

        for (props) |prop| {
            try properties.append(allocator, try parsePropertyDescriptor(allocator, prop));
        }
    ...
    pub fn releaseObject(self: *Self, object_id: []const u8) !void {
        try self.session.sendCommandVoid("Runtime.releaseObject", .{
            .object_id = object_id,
        });
    }
    ...
    pub fn releaseObjectGroup(self: *Self, object_group: []const u8) !void {
        try self.session.sendCommandVoid("Runtime.releaseObjectGroup", .{
        ...
        return switch (@typeInfo(T)) {
            ...
            .pointer => |ptr| {
                if (ptr.size == .slice and ptr.child == u8) {
                    return switch (value) {
                        .string => |s| try self.session.allocator.dupe(u8, s),
                        else => return error.TypeMismatch,
                    };
                }
...
pub const RemoteObject = struct {
    ...
    pub fn deinit(self: *Self, allocator: std.mem.Allocator) void {
        ...
        if (self.value) |*v| json_util.deinitValue(allocator, v);
...
fn parseRemoteObject(allocator: std.mem.Allocator, obj: std.json.Value) !RemoteObject {
    return .{
        ...
        .value = if (obj.object.get("value")) |v|
            try json_util.cloneValue(allocator, v)
        else
            null,
