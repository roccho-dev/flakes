...
pub const Response = struct {
    ...
    pub fn deinit(self: *Response, allocator: std.mem.Allocator) void {
        json_util.deinitValue(allocator, &self.result);
...
pub const Event = struct {
    ...
    pub fn deinit(self: *Event, allocator: std.mem.Allocator) void {
        ...
        json_util.deinitValue(allocator, &self.params);
        if (self.session_id) |sid| allocator.free(sid);
    }
};
...
pub fn parseMessage(allocator: std.mem.Allocator, raw_json: []const u8) !Message {
    ...
    if (value.object.get("id")) |id_val| {
        ...
        // Success response
        var result = if (value.object.get("result")) |result_val|
            try json_util.cloneValue(allocator, result_val)
        else
            json_util.emptyObjectValue(allocator);
        errdefer json_util.deinitValue(allocator, &result);
    ...
    if (value.object.get("method")) |method_val| {
        ...
        var params = if (value.object.get("params")) |params_val|
            try json_util.cloneValue(allocator, params_val)
        else
            json_util.emptyObjectValue(allocator);
        errdefer json_util.deinitValue(allocator, &params);
